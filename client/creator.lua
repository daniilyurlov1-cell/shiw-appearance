...
local function checkStrings(input)
    ...
    -- Запрещаем цифры и спецсимволы
    if string.match(input, '[%d%p]') then
        lib.notify({ title = locale('invalid_character_name.title'), description = locale('invalid_character_name.description'), type = 'error', duration = 7000 })
        return false
    end
    
    -- Разрешаем латиницу (с заглавной) или любую кириллицу
    local isLatin = string.match(input, '^%u%l*$')
    local hasCyrillic = string.match(input, '[а-яА-ЯёЁ]')
    
    if not isLatin and not hasCyrillic then
        lib.notify({ title = locale('invalid_character_name.title'), description = locale('invalid_character_name.description'), type = 'error', duration = 7000 })
        return false
    end
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'FirstMenu',
        {
            ...
            if data.current.value == 'save' then
                ...
                if Skinkosong then
                    ...
                    -- СОХРАНЯЕМ ОДЕЖДУ В ИНВЕНТАРЬ ПРИ РЕДАКТИРОВАНИИ
                    SaveCharacterClothingToInventory()
                ...
                elseif Firstname and Lastname and Nationality and Selectedsex and Birthdate and Cid then
                    ...
                    -- СОХРАНЯЕМ ОДЕЖДУ В ИНВЕНТАРЬ ПРИ СОЗДАНИИ ПЕРСОНАЖА
                    SaveCharacterClothingToInventory()
            ...
            -- Callback для закрытия меню (пустой)
        end)
end
...
-- ==========================================
-- СОХРАНЕНИЕ ОДЕЖДЫ В ИНВЕНТАРЬ ПРИ СОЗДАНИИ ПЕРСОНАЖА
-- ==========================================

function SaveCharacterClothingToInventory()
    -- Получаем ClothesCache из clothes.lua (глобальная переменная)
    local clothesCache = ClothesCache
    
    -- Если не нашли, пробуем через экспорт
    ...
    if not clothesCache or next(clothesCache) == nil then 
        print('[RSG-Clothing] No ClothesCache found')
        return 
    end
    ...
    if not success or not clothing then
        print('[RSG-Clothing] Could not load clothing data: ' .. tostring(err))
        return
    end
    ...
    for category, data in pairs(clothesCache) do
            ...
            if data.hash and data.hash ~= 0 then
                ...
                if clothing[gender] and clothing[gender][category] then
                    ...
                    if clothingTable and clothingTable[model] and clothingTable[model][texture] then
                        hash = clothingTable[model][texture].hash
                    end
                end
            end
        ...
        -- Добавляем если есть валидный хеш
        if hash ~= 0 then
            table.insert(itemsToSave, {
                ...
                texture = texture,
                isMale = isMale,
                price = 0 -- Бесплатно при создании
            })
            print('[RSG-Clothing] Character creation item: ' .. category .. ' | Hash: ' .. tostring(hash))
        end
        
        ::continue::
    end
    
    -- Отправляем все предметы на сервер
    if #itemsToSave > 0 then
        for _, item in ipairs(itemsToSave) do
            TriggerServerEvent('rsg-clothing:server:saveToInventory', item)
            Wait(100)
        end
        print('[RSG-Clothing] Saved ' .. #itemsToSave .. ' clothing items for new character')
    else
        print('[RSG-Clothing] No clothing items to save')
    end
end
